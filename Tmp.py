...
class Tomcat(object):
    ...
    def useage(self):
        ...
        print "~" * 50
        print "\033[32mTomcat Applications List:\033[0m" + "\033[31m1:%s 2:%s\033[0m" % (
            self.Tomcat_Examples[0], self.Tomcat_Examples[1])
        print "~" * 50
        print "\033[32m script use \033[0m" + "./" + scrip_name + "Tomcat Applications Name" + "\033[31m start|stop|restart|status|log\033[0m"
        print "or"
        print "\033[32m Script use \033[0m" + "python <path>/" + scrip_name + "Tomcat Applications Name" + "\033[31m start|stop|restart|status|log\033[0m"
...
if __name__ == '__main__':
    ...
    Handle = Tomcat(None)
    try:
        if sys.argv[1] in Handle.Tomcat_Examples:
            Handle = Tomcat(sys.argv[1])
        else:
            print "请输出正确的 Tomcat 实例名称"
        if len(sys.argv) < 3:
            Handle.useage()
            sys.exit()
